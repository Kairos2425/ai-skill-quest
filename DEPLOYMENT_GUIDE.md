...
1. GitHub 仓库建议设置为 Private。
2. 不要在前端提交 `.env`、API Key、Cloudflare Token、模型密钥、支付密钥。
3. 后续真实 AI 助教、用户系统、支付规则、课程权益判断必须放到后端。
4. 公开站点只部署 `dist/` 构建产物；不要公开原始课程材料和内部运营文档。

## 方式一：GitHub Pages

适合低成本托管。商业化项目建议使用私有仓库或只发布构建产物。
